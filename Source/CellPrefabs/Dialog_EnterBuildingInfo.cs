...
namespace CellPrefabs
{
    ...
    public class BuildingInfo
    {
        ...
        public bool exportNatural = false; // 新增：是否导出地形
        public bool exportPlant = true;    // 新增：是否导出植物
        public bool exportCreatureAndItem = false; // 新增：是否导出角色和物品
        public bool isAutoGenerated = true;
        public int needMoney = -1;
    }
    ...
    class Dialog_EnterBuildingInfo : Window
    {
        ...
        private string needMoney = "自动计算(AutoCalculate)"; // 建筑价格输入框
        private bool isAutoGenerated = false; // 自动生成标记
        private const string AutoKeywords = "自动|auto|Auto"; // 触发关键词
        ...
        private void LoadUserSettings()
        {
            ...
            exportNatural = settings.exportNatural;
            exportPlant = settings.exportPlant;
            exportCreatureAndItem = settings.exportCreatureAndItem;
        ...
        {
            var settings = SettingsManager.Settings;
            settings.exportNatural = exportNatural;
            settings.exportPlant = exportPlant;
            settings.exportCreatureAndItem = exportCreatureAndItem;
            settings.translateMode = translateMode;
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            //建筑所需白银
            Widgets.Label(new Rect(0, y, 100, 24), "建筑价格(Building Price):");
            needMoney = Widgets.TextField(new Rect(160, y, viewRect.width - 160, 30), needMoney);
            y += 40;
            ...
            // 新增：导出角色和物品勾选框
              Widgets.CheckboxLabeled(
                new Rect(0, y, viewRect.width, 30),
                "强制导出全部单位，物品,生物 (force export all thing)",
            ...
            if (Widgets.ButtonText(new Rect(inRect.width / 2 - 100, inRect.height - 50, 90, 30), "确定"))
            {
                if (ValidateInput())
                {
                    ...
                    // 保存勾选框状态
                    buildingInfo.exportNatural = exportNatural;     // 直接赋值
                    buildingInfo.exportPlant = exportPlant;         // 直接赋值
                    buildingInfo.exportCreatureAndItem = exportCreatureAndItem; // 直接赋值

                    buildingInfo.isAutoGenerated=isAutoGenerated;
        ...
        private bool ValidateInput()
        {
            ...
            if (Regex.IsMatch(needMoney, AutoKeywords))
            {
                isAutoGenerated = true;
            } else
            {
                if (!int.TryParse(needMoney, out int moneyValue) || moneyValue < 0)
                {
                    Messages.Message("建筑价格必须是有效的正整数!", MessageTypeDefOf.RejectInput);
                    return false;
                }
                isAutoGenerated = false;
                buildingInfo.needMoney = int.Parse(needMoney);
            }
        ...
        private static readonly List<string> ValidCategories = new List<string> {
    "AP_Category_Energy",        // 能源设施
    "AP_Category_Residence",     // 居住区
    "AP_Category_Defense",       // 防御工事
    "AP_Category_Anomaly",       // 异常管理
    "AP_Category_MyResearch",    // 科研设施
    "AP_Category_RoyalCulture",  // 贵族和文化
    "AP_Category_ProductionStorage", // 生产与仓储
    "AP_Category_AgricultureMedical", // 农业与医疗
    "AP_Category_TradeTransport", // 贸易和交通
    "AP_Category_BaseBlueprint",  // 基地蓝图
    "AP_Category_RapidConstruction", // 快速构建
    "AP_Category_Other"          // 其他
};
